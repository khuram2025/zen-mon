...
func (s *PostgresStore) LoadServiceChecks(ctx context.Context) ([]*checker.ServiceCheck, error) {
	rows, err := s.pool.Query(ctx, `
		SELECT sc.id, sc.device_id, sc.name, sc.check_type, sc.enabled,
		       ...
		       COALESCE(sc.http_ignore_tls_errors, false),
		       COALESCE(sc.http_allow_insecure_auth, false),
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			&sc.HTTPContentMatch, &sc.HTTPFollowRedirects, &sc.HTTPIgnoreTLSErrors,
			&sc.HTTPAllowInsecureAuth,
