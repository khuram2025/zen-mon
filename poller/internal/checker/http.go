...
func (c *HTTPChecker) Check(ctx context.Context, sc *ServiceCheck, pollerID string) *ServiceCheckResult {
	...
	if sc.CredentialID != nil && !strings.HasPrefix(strings.ToLower(sc.TargetURL), "https://") && !strings.EqualFold(sc.CredentialAuthType, "ntlm") && !sc.HTTPAllowInsecureAuth {
		result.Error = "basic, bearer, and form credentials require HTTPS unless trusted HTTP credential transmission is explicitly enabled"
		return result
	}
...
func (c *HTTPChecker) checkWorkflow(ctx context.Context, sc *ServiceCheck, pollerID string) *ServiceCheckResult {
	...
	if sc.CredentialID != nil && strings.ToLower(base.Scheme) != "https" && !strings.EqualFold(sc.CredentialAuthType, "ntlm") && !sc.HTTPAllowInsecureAuth {
		result.Error = "basic, bearer, and form credentials require HTTPS unless trusted HTTP credential transmission is explicitly enabled"
		return result
	}
...
func applyCredential(req *http.Request, sc *ServiceCheck, contentType string) error {
	...
	if !strings.EqualFold(req.URL.Scheme, "https") && !strings.EqualFold(sc.CredentialAuthType, "ntlm") && !sc.HTTPAllowInsecureAuth {
		return fmt.Errorf("basic, bearer, and form credentials require HTTPS unless trusted HTTP credential transmission is explicitly enabled")
	}
	switch strings.ToLower(sc.CredentialAuthType) {
	case "basic":
		req.SetBasicAuth(sc.CredentialUsername, sc.CredentialSecret)
	case "ntlm":
		// The Negotiator transport consumes this Basic header locally, performs
		// the NTLM challenge-response exchange, and never sends the plaintext
