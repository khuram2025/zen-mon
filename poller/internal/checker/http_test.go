...
func TestNTLMCredentialIsAllowedForTrustedInternalHTTP(t *testing.T) {
	credentialID := uuid.New()
	check := &ServiceCheck{
		CredentialID:       &credentialID,
		CredentialAuthType: "ntlm",
		CredentialUsername: "DOMAIN\\svc-monitor",
		CredentialSecret:   "not-sent-as-basic",
	}
	req, err := http.NewRequest(http.MethodGet, "http://iis.internal/protected", nil)
	if err != nil {
		t.Fatal(err)
	}
	if err := applyCredential(req, check, ""); err != nil {
		t.Fatalf("expected NTLM to be allowed for trusted internal HTTP: %v", err)
	}
	if req.Header.Get("Authorization") == "" {
		t.Fatal("expected credentials for the NTLM negotiator transport")
	}
}

func TestBasicCredentialStillRequiresHTTPS(t *testing.T) {
	credentialID := uuid.New()
	check := &ServiceCheck{
		CredentialID:       &credentialID,
		CredentialAuthType: "basic",
		CredentialUsername: "operator",
	...
	if err := applyCredential(req, check, ""); err == nil || !strings.Contains(err.Error(), "require HTTPS") {
		t.Fatalf("expected clear-text Basic authentication to be rejected, got %v", err)
	}
}

func TestBasicCredentialCanBeExplicitlyAllowedForTrustedHTTP(t *testing.T) {
	credentialID := uuid.New()
	check := &ServiceCheck{
		CredentialID:          &credentialID,
		CredentialAuthType:    "basic",
		CredentialUsername:    "operator",
		CredentialSecret:      "secret",
		HTTPAllowInsecureAuth: true,
	}
	req, err := http.NewRequest(http.MethodGet, "http://legacy.internal/protected", nil)
	if err != nil {
		t.Fatal(err)
	}
	if err := applyCredential(req, check, ""); err != nil {
		t.Fatalf("expected explicit trusted HTTP opt-in to allow Basic auth: %v", err)
	}
}
