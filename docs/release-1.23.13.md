...
The portal independently confirmed publication, exact package digest, minimum version 1.23.9 and full rollout without a target-group restriction. The development appliance receives the release offer, and its authenticated HTTPS download matched the package hash. No installation was triggered for this verification. The current updater does not enforce its saved automatic-update toggle or maintenance window; this existing policy defect is recorded in the publication report. This evidence does not claim fleet installation completion.
