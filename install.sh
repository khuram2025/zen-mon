...
SUDOEOF
    ...
    # Storage management (Settings -> Storage): root-owned copies of the
    # privileged scripts plus the sudoers policy that lets the API service
    # user run them. Root-owned installs under /usr/local/sbin are the
    # security boundary — never grant sudo on the zenplus-writable copies
    # in /opt/zenplus/bin.
    install -o root -g root -m 0755 /opt/zenplus/bin/zenplus-storage-helper /usr/local/sbin/zenplus-storage-helper
    install -o root -g root -m 0755 /opt/zenplus/bin/expand-storage.sh /usr/local/sbin/zenplus-expand-storage
    mkdir -p /data/backups/appliance /data/backups/clickhouse
    chown zenplus:zenplus /data/backups/appliance
    chmod 2775 /data/backups/appliance
    chown 101:101 /data/backups/clickhouse   # ClickHouse container uid
    cat > /etc/sudoers.d/zenplus-storage <<'SUDOEOF'
# Generated by ZenPlus installer — do not edit by hand.
...
# The helper scripts validate their arguments; keep the grants narrow.
zenplus ALL=(root) NOPASSWD: /usr/sbin/vgs *
zenplus ALL=(root) NOPASSWD: /usr/sbin/lvs *
zenplus ALL=(root) NOPASSWD: /usr/sbin/pvs *
zenplus ALL=(root) NOPASSWD: /usr/sbin/pvresize *
...
SUDOEOF
    chmod 0440 /etc/sudoers.d/zenplus-storage
    if visudo -cf /etc/sudoers.d/zenplus-storage >/dev/null 2>&1; then
        log "Storage sudoers rule installed and validated"
    else
        warn "storage sudoers validation failed — removing"
        rm -f /etc/sudoers.d/zenplus-storage
    fi
