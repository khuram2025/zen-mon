...
export function ErrorsInboxPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-[var(--text-primary)]">Errors</h1>
        <p className="text-sm text-[var(--text-muted)] mt-1">Grouped exceptions across all services, with triage.</p>
      </div>

      <div className="flex flex-wrap items-center gap-2">
        <button onClick={() => set('status', '')} className={`px-3 py-1.5 rounded-full text-xs border ${!status ? 'bg-[var(--accent)] text-white border-[var(--accent)]' : 'border-[var(--bg-elevated)] text-[var(--text-muted)]'}`}>
          All
        </button>
        {STATUSES.map((s) => (
          <button key={s} onClick={() => set('status', s)} className={`px-3 py-1.5 rounded-full text-xs border capitalize ${status === s ? 'bg-[var(--accent)] text-white border-[var(--accent)]' : 'border-[var(--bg-elevated)] text-[var(--text-muted)]'}`}>
            {s.replace(/_/g, ' ')} {counts[s] ? `(${counts[s]})` : ''}
          </button>
        ))}
        <div className="flex-1" />
        <select value={range} onChange={(e) => set('range', e.target.value)} className="h-9 rounded-md bg-[var(--bg-tertiary)] border border-[var(--bg-elevated)] text-sm px-2 text-[var(--text-primary)]">
      ...
      <Card>
        <CardContent className="p-0">
          {q.isLoading ? (
            <div className="flex items-center justify-center gap-2 text-[var(--text-muted)] py-12"><Loader2 className="w-4 h-4 animate-spin" /> Loading…</div>
          ) : issues.length === 0 ? (
            <div className="flex flex-col items-center gap-2 text-[var(--text-muted)] py-12"><Bug className="w-6 h-6" /> No errors in range.</div>
          ) : (
            <Table>
              ...
              <TBody>
                {issues.map((e) => (
                  <Tr key={e.group_id} className="cursor-pointer hover:bg-[var(--bg-tertiary)]" onClick={() => navigate(`/apm/errors/${e.group_id}`)}>
                    <Td>
                      <div className="font-medium text-[var(--text-primary)]">{e.exception_type}</div>
                      <div className="text-xs text-[var(--text-muted)] truncate max-w-md">{e.message}</div>
                    </Td>
                    ...
                    <Td className="text-right text-xs text-[var(--text-muted)]">{rel(e.last_seen)}</Td>
                  </Tr>
                ))}
              </TBody>
            </Table>
