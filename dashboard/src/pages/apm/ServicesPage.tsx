  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-[var(--text-muted)] mt-1">Golden-signal health for every instrumented application service.</p>
      </div>
      ...
      <Card>
        <CardContent className="p-0">
          {q.isLoading ? (
            ...
            <div className="flex flex-col items-center gap-2 text-[var(--text-muted)] py-12"><Boxes className="w-6 h-6" /> No services reporting. Send traces via the OTLP ingest.</div>
          ) : (
            <Table>
              <THead>
                <Tr>
              ...
              <TBody>
                {services.map((s) => (
                  <Tr key={s.name} className="cursor-pointer hover:bg-[var(--bg-tertiary)]" onClick={() => navigate(`/apm/services/${encodeURIComponent(s.name)}`)}>
                    ...
                    <Td className="text-xs text-[var(--text-muted)]">{s.envs.join(', ')}</Td>
                  </Tr>
                ))}
              </TBody>
            </Table>
