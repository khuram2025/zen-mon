...
export function ApmOverviewPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-semibold text-text">Application Monitoring</h1>
        <p className="text-sm text-muted mt-1">Fleet-wide application health from OpenTelemetry traces.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        <button onClick={() => navigate('/apm/services')} className="flex items-center gap-3 rounded-lg border border-border bg-surface px-4 py-3 hover:bg-surface2 text-left">
          <Boxes className="w-5 h-5 text-primary" /><div><div className="font-medium text-text">Services</div><div className="text-xs text-muted">RED dashboards & apdex</div></div>
        </button>
        <button onClick={() => navigate('/apm/slos')} className="flex items-center gap-3 rounded-lg border border-border bg-surface px-4 py-3 hover:bg-surface2 text-left">
          <Target className="w-5 h-5 text-primary" /><div><div className="font-medium text-text">SLOs</div><div className="text-xs text-muted">Error budgets & burn alerts</div></div>
        </button>
        <button onClick={() => navigate('/apm/service-map')} className="flex items-center gap-3 rounded-lg border border-border bg-surface px-4 py-3 hover:bg-surface2 text-left">
          <Network className="w-5 h-5 text-primary" /><div><div className="font-medium text-text">Service Map</div><div className="text-xs text-muted">Dependency topology</div></div>
        </button>
        <button onClick={() => navigate('/apm/traces')} className="flex items-center gap-3 rounded-lg border border-border bg-surface px-4 py-3 hover:bg-surface2 text-left">
          <GitBranch className="w-5 h-5 text-primary" /><div><div className="font-medium text-text">Traces</div><div className="text-xs text-muted">Search & waterfall</div></div>
        </button>
      </div>

      <Card>
        <CardHeader className="pb-1"><CardTitle className="flex items-center gap-2 text-sm"><AlertTriangle className="w-4 h-4 text-danger" /> Services needing attention</CardTitle></CardHeader>
        <CardContent className="p-0">
          {q.isLoading ? (
            <div className="flex items-center justify-center gap-2 text-muted py-10"><Loader2 className="w-4 h-4 animate-spin" /> Loading…</div>
          ) : worst.length === 0 ? (
            <div className="text-center text-muted py-10">No services reporting yet.</div>
          ) : (
            <Table>
