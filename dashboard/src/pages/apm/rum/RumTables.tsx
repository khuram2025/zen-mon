...
export function RumViewsTable({ data, onOpen, ...props }: SharedProps & { data?: RumListResponse<RumView>; onOpen: (row: RumView) => void }) {
  ...
  return (
    <RumTableCard title="Views" description="Route-level traffic, experience and error impact" notice={<RumCoverageNotice coverage={data?.coverage} />} footer={pageFooter(data, props, 'views')}>
      <Table>
        <THead><Tr>
        ...
        <TBody>
          {props.loading ? <LoadingRow columns={8} /> : !data?.items.length ? <EmptyRow columns={8} filtered={props.filtered} noun="views" icon={Layers3} /> : data.items.map((view) => (
            <Tr key={viewRowKey(view)} className={INTERACTIVE_ROW} tabIndex={0} onClick={() => onOpen(view)} onKeyDown={(event) => onRowKey(event, () => onOpen(view))}>
              <Td><div className="max-w-[300px] truncate font-mono text-xs font-medium text-text" title={view.view_name}>{view.view_name || '/'}</div><div className="text-[10px] text-muted">{view.application_id} · {view.env}</div></Td>
...
export function RumSessionsTable({ data, onOpen, ...props }: SharedProps & { data?: RumListResponse<RumSession>; onOpen: (row: RumSession) => void }) {
  ...
  return (
    <RumTableCard title="Sessions" description="User journeys with frontend and backend correlation" notice={<RumCoverageNotice coverage={data?.coverage} />} footer={pageFooter(data, props, 'sessions')}>
      <Table>
        <THead><Tr>
        ...
        <TBody>
          {props.loading ? <LoadingRow columns={8} /> : !data?.items.length ? <EmptyRow columns={8} filtered={props.filtered} noun="sessions" icon={Users} /> : data.items.map((session) => (
            <Tr key={session.session_id} className={INTERACTIVE_ROW} tabIndex={0} onClick={() => onOpen(session)} onKeyDown={(event) => onRowKey(event, () => onOpen(session))}>
              <Td><div className="font-mono text-xs font-medium text-text">{session.session_id.slice(0, 14)}…</div><div className="text-[10px] text-muted">{session.user_id || session.application_id} · {session.env}{session.service_version ? ` · ${session.service_version}` : ''}</div></Td>
              ...
              <Td className="text-right font-mono text-xs tabular-nums">{formatDurationMs(session.duration_ms)}</Td>
...
export function RumErrorsTable({ data, onOpen, ...props }: SharedProps & { data?: RumListResponse<RumError>; onOpen: (row: RumError) => void }) {
  ...
  return (
    <RumTableCard title="JavaScript errors" description="Grouped browser failures ranked by user impact" notice={<><RumCoverageNotice coverage={data?.coverage} /><RumSamplingNotice sampling={data?.sampling} /></>} footer={pageFooter(data, props, 'issues')}>
      <Table>
        <THead><Tr>
        ...
        <TBody>
          {props.loading ? <LoadingRow columns={7} /> : !data?.items.length ? <EmptyRow columns={7} filtered={props.filtered} noun="errors" icon={FileWarning} /> : data.items.map((error) => (
            <Tr key={errorRowKey(error)} className={INTERACTIVE_ROW} tabIndex={0} onClick={() => onOpen(error)} onKeyDown={(event) => onRowKey(event, () => onOpen(error))}>
              <Td><div className="max-w-[360px] truncate text-xs font-medium text-text" title={error.message}>{error.message}</div><div className="max-w-[360px] truncate font-mono text-[10px] text-muted">{error.error_type || error.source || error.fingerprint}</div></Td>
...
export function RumResourcesTable({ data, onOpen, ...props }: SharedProps & { data?: RumListResponse<RumResource>; onOpen: (row: RumResource) => void }) {
  ...
  return (
    <RumTableCard title="Resources" description="Fetch, XHR and static asset performance" notice={<RumCoverageNotice coverage={data?.coverage} />} footer={pageFooter(data, props, 'resources')}>
      <Table>
        <THead><Tr>
        ...
        <TBody>
          {props.loading ? <LoadingRow columns={8} /> : !data?.items.length ? <EmptyRow columns={8} filtered={props.filtered} noun="resources" icon={Network} /> : data.items.map((resource) => {
            ...
            return (
              <Tr key={resourceRowKey(resource)} className={INTERACTIVE_ROW} tabIndex={0} onClick={() => onOpen(resource)} onKeyDown={(event) => onRowKey(event, () => onOpen(resource))}>
                <Td><div className="flex items-center gap-1.5"><Badge variant="outline" className="uppercase">{resource.method || resource.resource_type || 'asset'}</Badge>{resource.status_code != null && <span className={resource.status_code >= 400 ? 'font-mono text-[10px] text-danger' : 'font-mono text-[10px] text-muted'}>{resource.status_code}</span>}</div><div className="mt-1 max-w-[320px] truncate font-mono text-[10px] text-text" title={resource.url || resource.name}>{resource.name || resource.url}</div></Td>
                ...
                <Td className="text-right font-mono text-xs tabular-nums">{formatDurationMs(resource.duration_p75)}</Td>
...
export function RumActionsTable({ data, onOpen, ...props }: SharedProps & { data?: RumListResponse<RumAction>; onOpen: (row: RumAction) => void }) {
  ...
  return (
    <RumTableCard title="User actions" description="Clicks, interactions and frustration signals" notice={<RumCoverageNotice coverage={data?.coverage} />} footer={pageFooter(data, props, 'actions')}>
      <Table>
        <THead><Tr>
        ...
        <TBody>
          {props.loading ? <LoadingRow columns={7} /> : !data?.items.length ? <EmptyRow columns={7} filtered={props.filtered} noun="actions" icon={MousePointerClick} /> : data.items.map((action) => {
            ...
            return (
              <Tr key={actionRowKey(action)} className={INTERACTIVE_ROW} tabIndex={0} onClick={() => onOpen(action)} onKeyDown={(event) => onRowKey(event, () => onOpen(action))}>
                <Td><div className="text-xs font-medium text-text">{action.name || 'Unnamed action'}</div><Badge variant={frustration ? 'warning' : 'outline'} className="mt-1">{action.action_type.replaceAll('_', ' ')}</Badge></Td>
                ...
                <Td className="text-right font-mono text-xs tabular-nums">{formatDurationMs(action.duration_p75)}</Td>
