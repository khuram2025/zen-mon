...
func TestLiveCaptureCollectsFlows(t *testing.T) {
	...
	for i, f := range final {
		...
		if i < 5 {
			t.Logf("  %s %s %s %s:%d -> %s:%d pid=%d proc=%q svc=%q sent=%d recv=%d known=%v state=%s",
				f.Protocol, f.Kind, f.Direction, f.LocalIP, f.LocalPort, f.RemoteIP, f.RemotePort,
				f.PID, f.ProcessName, f.ServiceName, f.BytesSent, f.BytesReceived,
				f.BytesKnown, f.State)
		}
	}
	t.Logf("flows carrying non-zero byte counts: %d/%d; protocols=%v kinds=%v",
		withBytes, len(final), protocols, kinds)

	for _, f := range final {
		switch f.Kind {
		case "connection":
			if f.RemoteIP == "" || f.RemotePort == 0 {
				t.Fatalf("connection has no peer: %+v", f)
			}
		case "listener", "endpoint":
			// Windows UDP owner tables and TCP listener tables expose only
			// the local bind. A blank peer is therefore expected and honest.
		default:
			t.Fatalf("unexpected flow kind %q: %+v", f.Kind, f)
		}
		switch f.Direction {
		case "inbound", "outbound", "local", "unknown":
		default:
			t.Fatalf("unexpected direction %q: %+v", f.Direction, f)
		}
		if f.Samples <= 0 {
	...
	if runtime.GOOS == "windows" {
		if protocols["udp"] == 0 {
			t.Fatal("Windows capture did not include any UDP endpoints")
		}
		if kinds["listener"] == 0 {
			t.Fatal("Windows capture did not include any TCP listeners")
		}
	}
}
...
func TestCollectorInterfaceFilterRetainsWildcardBinds(t *testing.T) {
	c := &collector{flows: map[flowKey]*Flow{}, baseline: map[flowKey][2]uint64{}}
	c.merge([]rawConn{
		{Protocol: "udp", Kind: "endpoint", Direction: "local", LocalIP: "0.0.0.0", LocalPort: 53, PID: 10},
		{Protocol: "udp", Kind: "endpoint", Direction: "local", LocalIP: "::", LocalPort: 5353, PID: 11},
		{Protocol: "tcp", Kind: "listener", Direction: "inbound", LocalIP: "192.0.2.25", LocalPort: 443, PID: 12},
	}, map[string]bool{"192.0.2.10": true}, 10)

	flows, count, _, _ := c.snapshot()
	if count != 2 || len(flows) != 2 {
		t.Fatalf("wildcard binds should survive an interface filter; got %+v", flows)
	}
	for _, flow := range flows {
		if !isUnspecifiedLocalIP(flow.LocalIP) {
			t.Fatalf("concrete address from another interface leaked through: %+v", flow)
		}
		if flow.Kind != "endpoint" || flow.Direction != "local" {
			t.Fatalf("flow classification was not preserved: %+v", flow)
		}
	}
}
