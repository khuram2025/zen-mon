...
root = Path('/tmp/zenplus-network-integration-20260910')
installed = Path('/opt/zenplus')
package = root/'candidate/update-1.23.11.zup'
expected = 'eb7737859c63cbe95890115a465e0ebf2d402bb96fbc16a83f9610586082776a'
assert os.getuid() == 0
assert hashlib.file_digest(package.open('rb'), 'sha256').hexdigest() == expected
assert (installed/'.version').read_text().splitlines()[0] == '1.23.10'
assert shutil.disk_usage(installed).free > 1500 * 1024 * 1024
extract = Path('/tmp/zenplus-updates/network-dev-1.23.11')
...
validate_manifest_transition(manifest, {'version':'1.23.11', 'min_version':manifest.get('min_version')},
    '1.23.10', (extract/'code/.version').read_text().splitlines()[0])
backup = installed/'updater/backups/network-dev-20260910-retry'
...
before = {'version':'1.23.10', 'services':{s:active(s) for s in services}, 'timer_active':timer_active}
...
try:
    with UpdateLock():
        ...
        with tarfile.open(backup/'local-configuration.tar.gz','w:gz') as tar:
            for name in ['/opt/zenplus/server/.env','/opt/zenplus/updater/config',
        ...
        add_record(version='1.23.11',from_version='1.23.10',status='applying',changelog='Approved local development canary; unpublished signed candidate.')
        executed = False
        try:
            ...
            (installed/'.version').write_text('1.23.11\n'+datetime.now(timezone.utc).isoformat()+'\n')
            add_record(version='1.23.11',from_version='1.23.10',status='success',changelog='Approved local development canary; unpublished signed candidate.')
            result={'ok':True,'from_version':'1.23.10','version':'1.23.11','package_sha256':expected,'backup':str(backup),'schema_ok':True}
            ...
            add_record(version='1.23.11',from_version='1.23.10',status='failed',error=str(exc),changelog='Local dev canary failed; inspect recovery evidence.')
