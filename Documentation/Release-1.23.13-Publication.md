...
The development appliance's automatic-update setting remains disabled, but inspection found that the current updater does not enforce that setting or the configured maintenance window. Its normal service invocation may apply an offered update; the UI's Check for Updates action starts that service. No installation was triggered during publication verification. This is a separate existing updater-policy defect, not an installation hold. Version prerequisites, signature/checksum verification and preflight checks still apply. This record establishes release availability and download integrity, not completed fleet installation or a canary soak.
