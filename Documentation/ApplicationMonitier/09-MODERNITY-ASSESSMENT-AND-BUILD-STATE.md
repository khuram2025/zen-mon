...
| AM-E6 Alerting + SLO | 9 metric keys in 4 places, evaluator loop, SLO burn loop | **Keys exist in the DB CHECK only** (1 of 4 places); no evaluator, no SLO loop; `apm_slos` dead schema | APM that cannot page anyone is a dashboard, not monitoring |
...
- **E-2 (AM-E6): make APM able to page someone.** Wire the 9 metric keys into the remaining 3 places (`alert_rules.py` allowlist, `alert_engine`, `AlertRuleFormDialog.tsx`); ship `apm_alert_evaluator_loop` + `apm_slo_burn_loop` as leader-elected loops (P0-T6 pattern, notification outbox from P1-T4). `apm_slos` stops being dead schema. *Without this, nothing else in APM matters operationally.*
