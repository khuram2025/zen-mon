...
from app.schemas.service_check import ServiceCheckCreate, ServiceCredentialCreate, ServiceCredentialResponse
from app.services.service_check_service import _validate_credential_transport
...
def test_basic_authenticated_workflow_requires_https():
    ...
    data = ServiceCheckCreate.model_validate(payload)
    with pytest.raises(ValueError, match="require HTTPS"):
        _validate_credential_transport(SimpleNamespace(auth_type="basic"), data.target_url, data.workflow_steps)


def test_basic_auth_can_be_explicitly_allowed_on_trusted_http():
    payload = _payload(target_url="http://portal.example.com/health")
    for step in payload["workflow_steps"]:
        step["url"] = step["url"].replace("https://", "http://")
    data = ServiceCheckCreate.model_validate(payload)
    _validate_credential_transport(
        SimpleNamespace(auth_type="basic"),
        data.target_url,
        data.workflow_steps,
...
def test_ntlm_is_allowed_for_trusted_internal_http():
    payload = _payload(target_url="http://portal.example.com/health")
    for step in payload["workflow_steps"]:
        step["url"] = step["url"].replace("https://", "http://")
    data = ServiceCheckCreate.model_validate(payload)
    _validate_credential_transport(SimpleNamespace(auth_type="ntlm"), data.target_url, data.workflow_steps)
    ...
    assert credential.auth_type == "ntlm"
